...
```{r derive-index, message=FALSE, warning=FALSE, error=FALSE, eval=FALSE, echo=FALSE}
ndvi = list.files("./cubes/2024_reg", 
  pattern = 'NDVI', full.names = T, all.files = FALSE)|>
  lapply(terra::rast)|>
  sprc() |>
...
ndbr = list.files("./cubes/2024_reg", 
  pattern = 'NDBR', full.names = T, all.files = FALSE)|>
  lapply(terra::rast)|>
  sprc() |>
  mosaic()
ndbr = terra::crop(ndbr, vect(aoi), mask=T)
ndbr = ndbr * 0.0001

writeRaster(ndvi, "./cubes/2024_mosaic/NDVI_2024_07.tif", overwrite=T)
writeRaster(ndbr, "./cubes/2024_mosaic/NDBR_2024_07.tif", overwrite=T)
```
...
```{r unsupervised-classifcation, eval=FALSE}
set.seed(99)
...
```{r kmean-viz, echo=FALSE, message=FALSE, warning=FALSE}
kmeans = rast("./cubes/2024_mosaic/NDVI_kmeans.tif")

tm_shape(kmeans) + 
  tm_raster(title = "KMeans Unsupervised, 2024", 
            palette = 'viridis') +
  tm_shape(aoi) + 
  tm_borders(col = "red") +
  tm_basemap("Esri.WorldImagery") -> map1
